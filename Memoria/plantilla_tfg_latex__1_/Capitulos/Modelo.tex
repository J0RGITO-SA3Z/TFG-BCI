...
El modelo MiRepNet proporciona la posibilidad de congelar los pesos del encoder y realizar fine-tuning únicamente sobre el tokenizador y la capa lineal de salida. Sin embargo, esta posibilidad no se menciona en ningún momento en el paper original [paper], ni tampoco se utiliza en los experimentos incluidos en el repositorio oficial del modelo.

Con el objetivo de evaluar el impacto de esta técnica sobre el rendimiento final, se llevó a cabo un experimento comparando ambos enfoques: realizar fine-tuning completo del modelo y realizar fine-tuning únicamente sobre las capas finales manteniendo congelado el encoder.

Para ello, los datos fueron divididos mediante validación cruzada k-fold, utilizando 10 epochs de entrenamiento en ambos casos. Posteriormente, se comparó el rendimiento obtenido al congelar el encoder frente al entrenamiento completo del modelo.

Los resultados finales del experimento se muestran en la Tabla \ref{tab:freezeComparison}, donde se observa una pérdida clara de rendimiento cuando el fine-tuning no se realiza sobre el encoder completo. Por lo tanto se ha optado por prescindir de este método. 
...
\begin{table}[h]
	...
	\begin{tabular}{lccc}
		...
		\hline
	\end{tabular}
	\caption{Accuracy (media $\pm$ desv. típ.) con y sin congelación del transformer durante el fine-tuning.}
