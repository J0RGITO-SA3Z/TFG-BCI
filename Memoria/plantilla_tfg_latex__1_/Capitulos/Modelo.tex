...
Con el objetivo de evaluar el impacto de esta técnica sobre el rendimiento final, se llevó a cabo un experimento comparando ambos enfoques: realizar fine-tuning completo del modelo y realizar fine-tuning manteniendo congelado el encoder.
...
Los resultados finales del experimento se muestran en la Tabla \ref{tab:freezeComparison}, donde se observa una pérdida clara de rendimiento cuando el fine-tuning no se realiza sobre el encoder. Por lo tanto se ha optado por prescindir de este método. 
...
Con el objetivo de determinar el número de trials por clase óptimo, se ha realizado un experimento fijando el número de epocas de entrenamiento a 10 , utilizando el preprocesado previamente descrito y clasificando unicamente mano derecha y mano izquierda. Para cada sujeto, se ha evaluado el rendimiento del modelo empleando diferentes cantidades de trials. Además, con el fin de reducir el impacto de la semilla aleatoria en la división de los datos entre entrenamiento y prueba, el proceso se ha repetido 10 veces para cada combinación de sujeto y número de trials, utilizando distintas semillas aleatorias. El resultado obtenido se muestra en la gráfica \ref{fig:numTrialsFinetuning}.
...
Por otro lado, los sujetos 2 y 4, cuyos resultados no superan aproximadamente el 55\% de accuracy en validación, comienzan a mostrar signos de overfitting a partir de las primeras epocas de entrenamiento. No obstante, dado el bajo rendimiento global de estos sujetos, no se han considerado determinantes para la selección final de hiperparámetros.

En el resto de sujetos, el sobreajuste comienza a apreciarse aproximadamente entre las epocas 6 y 10, aunque con distinta intensidad dependiendo del sujeto. En la mayoría de los casos, este fenómeno no provoca una caída significativa del accuracy de validación, sino más bien una estabilización de este mientras el rendimiento sobre entrenamiento continúa aumentando progresivamente.

Finalmente, se decidió utilizar 10 epocas durante el proceso de fine-tuning. Esta elección permite mantener un equilibrio entre el tiempo de entrenamiento y el rendimiento del modelo, evitando tanto un entrenamiento insuficiente como un sobreajuste excesivo.
