\chapter{Aplicación del sistema \textit{BCI}}
\label{cap:aplicacionBCI}

El primer paso para conseguir desarrollar un sistema que permita controlar un videojuego con el pensamiento es aprender a tratar con el casco y las señales EEG. 
...
\label{sec:funcionesPrincipales}
...
\section{Configuración del dispositivo EEG}
\label{sec:configuracionCasco}
...
\subsection{Configuración de acciones}
\label{subsec:configuracionAcciones}
...
\subsection{Ver batería del EEG}
\label{subsec:verBateria}
...
\subsection{Test de impedancias}
\label{subsec:testImpedancias}
...
\section{Grabación Manual}
\label{sec:grabacionManual}
...
\label{sec:visualizacionTiempoReal}
...
\section{Experimento Visual}
\label{sec:experimentoVisual}
...
\subsection{Funcionamiento de grabaciones}
\label{subsec:funcionamientoGrabaciones}
...
\label{subsec:protocoloExperimento}

El protocolo define la estructura temporal del experimento: el número de repeticiones de imaginación motora (\textit{trials}) por clase,que clases aparecerán (mano izquierda, mano derecha y pies), la duración de los periodos de imaginación motora, los intervalos de descanso entre \textit{trials} y las instrucciones presentadas al sujeto durante cada fase. Todos estos parámetros son configurables, lo que permite adaptar el experimento de forma flexible a las necesidades específicas de cada sesión o participante.

\section{Evaluación de sujetos con el modelo MiRepNet}
\label{sec:evaluacionMiRepNet}

Este módulo automatiza la evaluación del rendimiento del sistema sobre las grabaciones de un sujeto. El usuario puede seleccionar, para un sujeto determinado, las grabaciones concretas que desea incluir en la evaluación, lo que permite analizar el rendimiento y la precisión (\textit{accuracy}) obtenidos en cada conjunto e identificar qué condiciones o sesiones arrojan mejores resultados. Para ello, se aplica el \textit{pipeline} completo de preprocesado, que incorpora una etapa de detección y descarte de \textit{trials} ruidosos, se realiza el ajuste fino (\textit{fine-tuning}) del modelo predictor MiRepNet sobre los datos del sujeto y se calculan métricas de clasificación mediante validación cruzada, lo que facilita la comparación del desempeño entre distintos sujetos y sesiones. Una vez finalizada la evaluación, los resultados se presentan de forma visual mediante un conjunto de gráficas que incluyen la evolución de la precisión y del \textit{learning rate} a lo largo del entrenamiento, la matriz de confusión, un diagrama de cajas y bigotes y un diagrama de barras con la \textit{precision}, \textit{recall} y \textit{accuracy} desglosados por clase, proporcionando una visión completa del comportamiento del modelo (véase Figura~\ref{fig:modeloGraficoFinetune} y Figura~\ref{fig:modeloGraficoMatriz}).
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Bitmap/modelo_grafico_1.png}
    \caption{Gráficos del resultado de evaluación con \textit{accuracy}, \textit{loss} y \textit{learning rate} por época.}
...
\begin{figure}[h]
    ...
    \caption{Matriz de confusión del resultado del \textit{fine-tuning} y validación del modelo MiRepNet, gráfico con \textit{accuracy}, \textit{precision} y \textit{recall}, y diagrama de bigotes o cajas.}
    \label{fig:modeloGraficoMatriz}
\end{figure}

\section{Experimento en Tiempo Real}
\label{sec:experimentoTiempoReal}

El módulo de Experimento en Tiempo Real constituye la implementación completa del \textit{pipeline} de interfaz cerebro-computador, integrando de las etapas de adquisición de señal EEG, filtrado, normalización e inferencia del modelo predictor. El resultado es un sistema capaz de traducir la imaginación motora del sujeto en acciones concretas sobre un entorno de \textit{benchmarking} o videojuego. Para garantizar un rendimiento adecuado, el módulo incorpora una fase previa de entrenamiento idéntica a la del Experimento Visual, que permite ajustar el modelo predictor a las características individuales del sujeto antes de iniciar la sesión en tiempo real.
...
\item \textbf{NumPy:} biblioteca utilizada para computación numérica y manipulación eficiente de \textit{arrays} multidimensionales. Se emplea para el procesamiento y normalización de las señales EEG antes de su entrada al modelo.

\item \textbf{PyTorch:} \textit{framework} de aprendizaje profundo empleado para la carga, el ajuste fino (\textit{fine-tuning}) y la inferencia del modelo MiRepNet (véase Sección~\ref{sec:evaluacionMiRepNet}). Permite aprovechar la GPU mediante CUDA cuando está disponible, lo que acelera significativamente el proceso de entrenamiento.

\item \textbf{PyQt5 / pyqtgraph:} \textit{frameworks} utilizados para la implementación de la aplicación de visualización TCP en tiempo real, que permite monitorizar las señales EEG desde la misma máquina o desde un dispositivo externo durante el transcurso de una grabación o experimento (véase Sección~\ref{sec:visualizacionTiempoReal}).

\item \textbf{Pygame:} librería empleada con dos propósitos principales: la implementación del experimento visual y la presentación de los estímulos visuales en pantalla (véase Sección~\ref{sec:experimentoVisual}), y el desarrollo de los juegos y \textit{benchmarks} utilizados en el experimento en tiempo real (véase Sección~\ref{subsec:juegosBCI}).

\end{itemize}

\subsection{Resolución de conflictos con multihilo}
...
\subsection{Instalación y entorno}
...
\subsection{Inicialización de la aplicación}
