...
CSV_PATH = os.path.join(
    ...
    "[NEW]ScorePonderado_2_6", "SalidaPredicciones.csv"
)
FIF_PATH = os.path.join(
    PROJECT_ROOT, "recordings", "simulations_RT",
    "[NEW]ScorePonderado_2_6", "suj2_6_raw.fif"
)
...
def visor_score_ponderado(
    ...
    required = {"sample", "prediction", "p_left_raw", "p_right_raw", "score_total", "mean_score"}
    ...
    # ── figura: 2 paneles ─────────────────────────────────────────────────
    fig, (ax_probs, ax_score) = plt.subplots(2, 1, figsize=(15, 7), sharex=True)
    fig.suptitle("Score Ponderado  |  score > 0 → right_hand,  score < 0 → left_hand", fontsize=12)
    ...
    # panel inferior: score acumulado de la ventana ───────────────────────
    _shade_trials(ax_score, event_samples, event_desc, trial_samples)
    _shade_predictions(ax_score, samples, predictions)

    ax_score.fill_between(samples, score_total, 0,
                          where=(score_total >= 0), color="tab:orange", alpha=0.25, label="score → right")
    ax_score.fill_between(samples, score_total, 0,
                          where=(score_total < 0),  color="tab:blue",   alpha=0.25, label="score → left")
    ax_score.plot(samples, score_total, color="black",    linewidth=1.4, label="score_total")
    ax_score.plot(samples, mean_score,  color="tab:purple", linewidth=1.0,
                  linestyle="--", alpha=0.7, label="mean_score (confianza media)")
    ax_score.axhline(0, color="gray", linestyle=":", linewidth=0.8, alpha=0.5)
    _draw_event_lines(ax_score, event_samples, event_desc)
    ax_score.set_ylabel("Score de ventana")
    ax_score.set_xlabel("Número de muestra")
